 ...
 */
static int viommu_del_mappings(struct viommu_domain *vdomain, u64 iova, u64 end)
{
	size_t unmapped = 0;
	unsigned long flags;
			       ...
			       size_t len)
{
	...
	/* Keep the list sorted. Don't bother with overlap, it's forbidden. */
	list_for_each_entry(next, &vdev->resv_regions, list) {
		if (next->start > region->start)
			break;
	}
	list_add(&region->list, next->list.prev);
	return 0;
}
